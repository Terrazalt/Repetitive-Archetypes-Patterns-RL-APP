<script lang="ts">
	...
	async function sendAsFormData(dataUrl: string): Promise<Blob> {
		...
		const res = await fetch(env.YOLO_ENDPOINT, {
			method: 'POST',
			headers: {
				Authorization: `Bearer ${env.YOLO_API_KEY}`
			},
			body: form
		});
	...
	async function getBoundingBoxes(dataURL: string) {
		const blob = await (await fetch(dataURL)).blob();
		const form = new FormData();
		form.append('image', blob, 'scan.png');
		const res = await fetch(env.BOUNDING_BOXES_ENDPOINT, {
			method: 'POST',
			headers: {
				Authorization: `Bearer ${env.YOLO_API_KEY}`
			}
		});
		if (!res.ok) throw new Error(`Bounding Boxes API Error: ${res.statusText}`);
